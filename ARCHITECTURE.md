...
- **Screens**: Discrete full-page views.
    ...
    - `SubscriptionListScreen`: Dedicated interface for pass management with high-performance search and subscription-based filtering. Cards automatically highlight passes that have active subscriptions for the "Current Meal" and provide integrated **WhatsApp/Call shortcuts** for registered residents.
    - `DashboardScreen`: Aggregated analytics with a sleek, earthy-toned financial summary. Features a dynamic "Current Meal" demand snapshot and supports visual comparative bar charts.
    - `QrScreen`: Renders a permanent digital pass with seasonal branding. Used for verification during meal collection.
    - `DetailsScreen`: Comprehensive view of a single pass including food plan and collection matrix. Integrates **Quick Contact Actions** (WhatsApp/Call) directly into the Pass Identity card for residents with registered mobile numbers.
