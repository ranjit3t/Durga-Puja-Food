...
- **Screens**: Discrete full-page views.
    ...
    - `ScannerScreen`: A dual-mode validation interface. It utilizes the camera for QR scanning and provides a dedicated numeric input at the top for manual **4-digit Passcode** entry. Validating either the QR or the Passcode triggers a deep-link to the resident's pass details.
    - `QrScreen`: Renders a permanent digital pass with seasonal branding. Used for verification during meal collection. Uses grammar-aware headcount labels. Now displays the unique **4-digit Passcode** in bold blue text for manual validation fallback.
...
- **Data Normalization**: Handles schema variations and ensures data matrix integrity (Person x Day x Meal). Manages the migration and generation of unique **4-digit Passcodes** for every registration.
