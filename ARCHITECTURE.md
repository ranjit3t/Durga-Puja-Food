...
- **Smart Sorting**: 
    - **Operational Views**: Prioritize the "Current Meal" by floating the active day to the first position.
    - **Form Views**: Maintain chronological stability but use automated focus and horizontal scrolling to highlight the active window.
- **Future Protection**: When a "Current Meal" is globally active, the system automatically blocks updates to the `taken` status of all future meal slots. Users can only modify collection records for the current service window and historical meals.
