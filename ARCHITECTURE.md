...
- **Screens**: Discrete full-page views.
    ...
    - `SubscriptionListScreen`: Dedicated interface for pass management with high-performance search and subscription-based filtering. Cards automatically highlight passes that have active subscriptions for the "Current Meal" with a distinct "Meal Subscribed" marker and icon.
    - `DashboardScreen`: Aggregated analytics with a sleek, earthy-toned financial summary (conditional) and real-time operational metrics. Features a high-visibility summary card with seasonal totals and a dynamic "Current Meal" demand snapshot for the active service slot, including single-diet (Veg/Non-Veg Only) badges; the card acts as a navigation shortcut to the `ReportScreen`. Guest plate counts are read-only and derived from the `GuestManagementScreen`, with aggregated "Guest Total" and "Guest Taken" summaries displayed in dual-diet mode. Individual meal sections are clearly demarcated with borders, and completed service slots are visually dimmed to signify inactivation. Each meal section uses a standardized metric grid where the "Total Taken" field is prioritized as the final summary item.
