...
- **Contextual Pass Highlighting & Filtering**: The `SubscriptionListScreen` implements real-time subscription detection and filtering. It cross-references each pass's `mealSlots` with the globally active "Current Meal".
  - **Markers**: If any member within a pass is registered for the current service window, the pass card is decorated with a specialized **"Meal Subscribed"** marker for immediate operational verification.
  - **Quick Filters**: When a meal is live, the screen offers "All" and "Meal Subscribed" toggle filters, allowing volunteers to instantly isolate current meal recipients from the total subscription pool.
...
- **Integrated Resident Communication**: Both the `SubscriptionListScreen` and `DetailsScreen` leverage the `Linking` API to provide direct communication paths.
  ...
  - **Placement**: Icons are positioned at the bottom of pass cards in the subscription list, and within the **Pass Identity card** bottom section in the details view for high-density access.
...
- **Dual Visualization (Grid/Chart)**: Within each meal section, the **Total Taken** metric is strategically placed at the end of the grid to serve as the final reconciliation anchor. Volunteers can toggle between a numeric `MealMetricGrid` and a visual `MealBarChart` using the bottom-left action bar. The bar chart provides a comparative view of "Planned" vs "Taken" plates for each dietary type, along with Guest and Parcel counts.
- **Direct PNG Share (WhatsApp)**: Each meal section is equipped with a theme-aware WhatsApp export handler located at the bottom-right. It captures the current state (Grid or Chart) as a high-quality PNG image, ensuring operational data is communicated accurately to external teams.
