...
- **Activity Log & Audit Trail**: A comprehensive, permanent history of all operations. Admins can access a **Live Audit Dashboard** (auto-refreshing every 10 seconds with heartbeat indicator) with advanced search and multi-criteria filtering (**User/Module/Target/Date/Errors**). The system captures **Rich Transaction Metadata** including detailed pass demand breakdowns, collection history, granular configuration changes (global flags, payment methods, day rules), and per-item menu evolution (dishes added/removed, price adjustments). Includes **Hardware Fingerprints** (OS/Version) and **Application Versions** to track lifecycle changes. Features **Deep-Linking**—clicking a pass log instantly navigates to its current details. Includes one-tap **Forensic Export** to text files and toggleable **Chronological Sorting**.
