...
- **Dedicated Pass List**: A standalone **Subscriptions** screen centralizes pass discovery with high-performance search and **Natural Alphanumeric Sorting**. The interface features a **Dynamic Filter Bar** that intelligently appears only when relevant operational filters (**Current Meal**, **Current Meal Missed**, **Kids**, **Parcels**, or **Veg Only**) are available. Pass cards display visual markers—restaurant icon for active subs, red circular badges for missed meals, happy icon for kids, briefcase icon for parcels, and a **leaf icon** for strictly vegetarian passes—for rapid scanning. Includes **Direct Contact Icons** (WhatsApp and Phone) for instant communication.
- **Digital Registration**: Register flats with block number, flat number, and headcount. **Flat Number is a mandatory field**. Supports optional **Mobile Number** registration with a native **Contact Picker** for fast entry. Features **Automated Food Pricing** that pre-calculates the subscription amount based on member choices and **Additional Parcel Surcharges**. Includes an **Interactive Member Counter** for quick headcount adjustments. Supports separate **Adult and Kids** counts (when enabled). **Block and Flat numbers are locked during editing** to maintain QR identity stability.
...
- **Intelligent Focus & Scrolling**: The system maintains operational continuity by automatically selecting and focusing on the **Current Meal** when opening reports (specifically "Meal Not Taken" and "Kids Meal") or pass forms, minimizing manual navigation during busy service windows.
...
- **Sequential Collection Tracking**: staff first mark a meal as "Taken" (dine-in), which then unlocks a secondary toggle to record "Parcel Collection" if takeaway was registered. To maintain data integrity, the system enforces a strict dependency: a parcel cannot be marked as "Taken" until the primary dine-in meal has been recorded. Furthermore, unmarking the primary meal automatically clears any recorded parcel collection for that slot. During editing, the system provides a **Parcel Inconsistency Alert** if a member has opted for a parcel but only the primary meal was marked as taken, ensuring complete data entry.
