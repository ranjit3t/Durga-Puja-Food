...
- **Smart Navigation Strategy**: The system uses a dual-sorting strategy: **Operational screens** (Dashboard/Guest Management) always prioritize the "Current Meal" by moving it to the top. **Data Entry forms** maintain a fixed chronological order (Day 1, Day 2, etc.) while using **Intelligent Auto-Focus** to automatically select and scroll the active day into view.
...
- **Current Meal Lock**: To maintain data accuracy during live service, when a "Current Meal" is globally active, the system automatically disables the "Taken" and "Parcel" toggles for all **Future Meals** across the festival duration. Only the current and historical meals remain editable, preventing erroneous forward-dated entries.
...
- **Kitchen Dashboard**: Real-time operational data showing exactly how many plates and parcels are needed. Terminology intelligently simplifies on single-diet days. On **Web**, all screens scale to **100% browser width** for maximum high-density visibility.
- **Activity Log & Audit Trail**: A comprehensive, permanent history of all operations. Admins can access a **Live Audit Dashboard** (auto-refreshing every 10 seconds with heartbeat indicator) with advanced search and multi-criteria filtering (**User/Module/Target/Date/Errors**). The system utilizes the **Festive Color-Coded Card System** for log items, ensuring visual consistency with the rest of the application. It captures **Rich Transaction Metadata** including detailed pass demand breakdowns, collection history, granular configuration changes, and per-item menu evolution. Includes **Hardware Fingerprints** and **Application Versions**. Features **Deep-Linking**—clicking a pass log instantly navigates to its details. Includes one-tap **Forensic Export** and toggleable **Chronological Sorting**.
