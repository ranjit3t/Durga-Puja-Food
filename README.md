...
- **Real-Time Summary Card**: The Home screen summary card features a high-density, space-saving horizontal layout that neatly groups active passes, total members (adults/kids), and guest counts (season/current meal). It dynamically displays cumulative collection data when enabled, alongside an interactive deep-linking **LIVE** service shortcut badge. The card also features a large, centered background watermark icon for a premium look.
